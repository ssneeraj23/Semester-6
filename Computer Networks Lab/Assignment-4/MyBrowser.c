...
int recv_cli(int newsockfd, struct http_request *hr)
{
    ...
    printf("\n-----Response received for above request is-----\n");
    printf("%s", buf);
...
void send_put(struct http_request *req, int sockfd, char *buf)
{
    ...
    if (pol <= 0)
    {
        printf("Timedout and closing the connection\n ");
        return;
    }
    //recv(sockfd, put_resp, response_size, 0);
    recv_chunk(put_resp,sockfd);
    printf("\n-----Response received for above request is-----\n");
    printf("%s\n", put_resp);
    return;
}
